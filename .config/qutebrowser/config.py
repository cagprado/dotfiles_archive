...
c.auto_save.interval = 15000               # time (ms) between auto_save config/cookies
c.auto_save.session = True                 # restore session on startup
c.backend = 'webkit'                       # webengine (chromium, preferred) | webkit (safari)
c.confirm_quit = ['downloads']
c.editor.command = ['terminal', '-e', 'vim {file} -c "normal {line}G{column0}l"']
c.editor.encoding = 'utf-8'

c.content.autoplay = True                  # autoplay videos
c.content.cache.appcache = True            # HTML5 cache
c.content.cache.maximum_pages = 10         # cache loaded pages for nicer user experience
c.content.cache.size = None                # None = use default value
c.content.canvas_reading = True            # needed for some websites
c.content.cookies.accept = 'all'           # which cookies to accept
c.content.cookies.store = True             # store cookies
c.content.default_encoding = 'iso-8859-1'  # default encoding for websites
c.content.desktop_capture = 'ask'
c.content.dns_prefetch = True
c.content.frame_flattening = False         # flatten frames to a single scrollable page
...
c.content.images = True                    # autoload images
...
c.content.netrc_file = None                # None = uses '~/.netrc'
...
c.downloads.location.suggestion = 'both'   # path/filename
c.downloads.open_dispatcher = None
c.downloads.position = 'bottom'
c.downloads.remove_finished = -1

config.load_autoconfig()         # load settings done via GUI (at end of config so it will overwrite this file settings)
...
c.url.searchengines = {
    ...
    'tr'        : 'https://fanyi.baidu.com/#zh/en/{}',
}
...
config.bind(',tt', 'open -t tr {url}')
config.bind(',tg', 'open -t https://translate.google.cn/translate?hl=&sl=auto&tl=en&u={url}')
